...
public class Main {
    ...
    public static int CalculateNavigateTransfer(String source, String target){
        ...
        total = navigate(target,or2.get(con))+ navigate(source,or2.get(con))-2;
        return  total;
    }

    // this function is generates an orbit list from source to target
    public static ArrayList<String> navigateList(String source, String target){
        ArrayList<String> list= new ArrayList<>();
        String or=source;
        list.add(or);
        while (!or.equals(target)){
            int l= location(or);
            or=orbit1.get(l);
            list.add(or);
        }
    ...
    private static int CalculateTotal(String targetOrbit) {
        int total=0;
        for (String or: orbits
        ) {
            total+=navigate(or,targetOrbit);

        }
        return total;
    }

    public static int navigate(String orbit,String target){
        int step=0;
        String or=orbit;
        while (!or.equals(target)){
            int l= location(or);
    ...
    public static int location(String orbit){
        int loc=0;
            for (int i=0;i<orbit1.size();i++){
                if(orbit2.get(i).equals(orbit))
                    loc=i;
            }
        return loc;
    }

    private static void GetOrbits(){
        for (String s: list) {
            String[] sub = s.split("\\)");
            orbit1.add(sub[0]);
            orbit2.add(sub[1]);
